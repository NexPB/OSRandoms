...
public class MasterRandoms extends GraphScript<RandomContext> implements PaintListener {
	...
	public MasterRandoms() {
		chain.addAll(Arrays.asList(
				new Molly(ctx), new Certer(ctx), new QuizMaster(ctx), 
				new StrangePlant(ctx), new StrangeBox(ctx), new ExpRewardClaimer(ctx), 
				new TalkToRandoms(ctx), new Pinball(ctx), new SandwichLady(ctx), 
				new Mime(ctx), new DrillDemon(ctx)
			)
		);
	}
